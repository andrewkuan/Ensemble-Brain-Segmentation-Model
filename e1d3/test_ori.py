    ...
    def inference_loop(self, data, weight):
        ...
        with torch.no_grad():
            for loop in range(batches):
                ...
                with torch.no_grad():
                    # prediction:
                    output_tensor = self.model(torch.from_numpy(batch_data).cuda(non_blocking=True))

                print("Output Size:" + str(output_tensor.size()))

                probabilities_wt = F.softmax(output_tensor, dim=1)
                probabilities_tc = F.softmax(output_tensor, dim=1)
                probabilities_en = F.softmax(output_tensor, dim=1)
